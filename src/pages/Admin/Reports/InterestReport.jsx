        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="text-left">
                    <h2 className="text-3xl font-black text-gray-900 italic uppercase tracking-tight">Pendapatan Bunga</h2>
                    <p className="text-sm text-gray-500 mt-1 font-medium italic uppercase tracking-wider">Laporan Porsi Bunga Pinjaman</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 border-b border-gray-100 italic font-black text-[10px] uppercase tracking-widest text-gray-400">
                                ...
                                <th className="px-6 py-4 text-right">Porsi Bunga</th>
                                ...
                                paginatedData.map((item, idx) => (
                                    <tr key={item.id} className="hover:bg-emerald-50/20">
                                        ...
                                        <td className="px-6 py-4 text-left">
                                            <p className="text-xs font-black text-gray-900 uppercase italic">{item.nama}</p>
                                            <p className="text-[10px] text-gray-400 font-mono tracking-tighter">{item.no_pinjaman}</p>
                                        </td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="px-2 py-1 bg-gray-100 rounded text-[10px] font-black text-emerald-600">#{item.bulan_ke}</span>
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <span className="text-xs font-black text-emerald-600 font-mono italic">{formatCurrency(item.interest_amount)}</span>
                                        </td>
                                        <td className="px-6 py-4 text-center text-[10px] font-bold text-gray-500 italic">
                                            {item.tanggal_bayar ? new Date(item.tanggal_bayar).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'}
                                        </td>
                                    </tr>
                                ))
                            )}
